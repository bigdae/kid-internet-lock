            ...
            WM_COMMAND => {
                ...
                if notify == EN_SETFOCUS && !ctx_ptr.is_null() {
                    ...
                    if ctx.config.verify_password(&input) {
                        ...
                        let err_text = format!("{}\0", lang::PWD_INCORRECT).encode_utf16().collect::<Vec<u16>>();
...
pub fn prompt_admin_password(parent_hwnd: HWND, config: &AppConfig, title: &str) -> bool {
    ...
    unsafe {
        ...
        // Instruction label
        let label_text: Vec<u16> = format!("{}\0", lang::PWD_PROMPT_LABEL).encode_utf16().collect();
        ...
        // OK Button
        let ok_text: Vec<u16> = format!("{}\0", lang::BTN_OK).encode_utf16().collect();
        ...
        // Cancel Button
        let cancel_text: Vec<u16> = format!("{}\0", lang::BTN_CANCEL).encode_utf16().collect();
